...
class BatchAllTripletLoss(torch.nn.Module):
    ...
    def _euclidean_distance_matrix(self, embeddings): #:(self, embeddings:torch.Tensor)->torch.Tensor:  
        ...
        distance_matrix = torch.nn.functional.relu(distance_matrix)
    ...
    def forward(self, pred_embeddings:torch.Tensor, target_labels:torch.Tensor):
        ...
        distance_matrix = self._euclidean_distance_matrix(pred_embeddings)
        ...
        valid_triplets = (triplet_loss > self.eps).to(torch.float)
        num_positive_triplets = torch.sum(valid_triplets)

        triplet_loss = triplet_loss.sum() / (num_positive_triplets + self.eps)

        num_valid_triplets = torch.sum(mask)
        fraction_positive_triplets = num_positive_triplets / (num_valid_triplets + self.eps)
