...
class OnlineTripleLoss(nn.Module):
    def __init__(self, margin, sampling_strategy="fixed_sh"):
    ...
    def forward(self, embeddings, labels):
        ...
        # print("loss :", ap_dists - an_dists + self.margin + 1e-8)
        loss = F.relu(ap_dists - an_dists + self.margin + 1e-8)
...
class NegativeTripletSelector:
    ...
    def get_triplets(self, embeddings, labels):
        ...
        unique_labels, counts = torch.unique(labels.to("cpu"), return_counts=True)
        triplets_indices = [[] for i in range(3)]
        for i, label in enumerate(unique_labels):
            label_mask = labels == label
            label_indices = torch.where(label_mask.to("cpu"))[0]
